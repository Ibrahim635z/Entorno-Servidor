<?php
require_once("control.php");

// un array estático donde guardamos solamente el nombre/descripción del artículo
$articulos = ["Toalla", "Raquetas", "Pendrive", "Libreta"];
?>

<body>
    <div>
        <h3>Artículos</h3>
        <form>
            <ul> <!-- cada artículo será un elemento de lista -->
                <!-- recorremos los articulos -->
                <?php foreach ($articulos as $articulo): ?>
                    <!-- Creamos el elemento de lista -->
                    <li>
                        <!--
y dentro de cada elemento un botón con el nombre artículo y con valor
el nombre/descripción del artículo.
Cada vez que pulsemos un botón enviaremos el nombre o descripción del artículo
-->
                        <button type="submit" value="<?php echo $articulo ?>" name="articulo">
                            <?php echo $articulo ?>
                        </button>
                    </li>
                <?php endforeach; ?>
            </ul>
        </form>
    </div>
    <!-- Para poder ver la cesta -->
    <div>
        <h3>Cesta</h3>
        <!-- Creamos una lista para mostra los artículos guardados en la cesta -->
        <ul>
            <!-- Recorremos la variable de sesión 'cesta' que es un array -->
            <?php foreach ($_SESSION["cesta"] as $articulo): ?>
                <!-- Creamos el elemento de lista -->
                <li>
                    <!-- mostramos el elemento de la cesta -->
                    <?php echo $articulo; ?>
                </li>
            <?php endforeach; ?>
        </ul>
    </div>
</body>